    ...
    IPointGuardTaskManager
{
    ...
    function resolveChallenge(uint32 referenceTaskIndex, bool isChallengeValid)
        external
        onlyOwner
    {
        require(
            taskSuccesfullyChallenged[referenceTaskIndex] == true,
            "The response to this task has not been challenged successfully."
        );

        emit TaskChallengeResolved(referenceTaskIndex, isChallengeValid);
    }
